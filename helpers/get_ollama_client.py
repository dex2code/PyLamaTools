...
def get_ollama_client(settings: SettingsModel) -> ollama.Client:
    ...
    try:
        ...
        logger.exception(
            f"Ошибка подключения к API Ollama по адресу {settings.ollama_url}"
        )
    ...
    if settings.ollama_model not in model_names:
        logger.error(
            f"Модель, указанная в настройках ({settings.ollama_model}), "
    ...
    logger.info(
        f"Успешно подключились к Ollama API {settings.ollama_url} "
